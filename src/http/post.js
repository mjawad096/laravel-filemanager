...
export default {
   ...
   * @returns {*}
   */
  protectedUrl(disk, items, options) {
    options = options || {};
    let password = options.password || null;
    let password_confirmation = options.password_confirmation || null;
    let expire_at = options.expiry || null;
    return HTTP.post('protected-url', {disk, items, password, password_confirmation, expire_at});
  },
