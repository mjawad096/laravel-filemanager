<template>
	<div v-if="isLoggedIn" class="fm d-flex flex-column" v-bind:class="{ 'fm-full-screen': fullScreen }">
		<navbar></navbar>
		<div class="fm-body">
			<notification></notification>
			<context-menu></context-menu>
			<modal v-if="showModal" ref="modal"></modal>
...
export default {
	...
	created() {
		...
		*/
		...
		document.addEventListener('dragenter', (event)=>{
			dragging++;
			if(dragging == 1){
				this.$store.commit('fm/modal/setDraggingState', true);
		...
		document.addEventListener('dragleave', (event)=>{
			dragging--;
	  		if (dragging === 0) {
				this.$store.commit('fm/modal/setDraggingState', false);
		...
		document.addEventListener('drop', (event)=>{
			...
			window.test = this;
			this.$refs.modal.$children[0].newFiles = event.dataTransfer.files;
	...
	computed: {
		isLoggedIn(){
			return this.$store.state.fm.auth.loggedIn;
		},
		...mapState('fm', {
