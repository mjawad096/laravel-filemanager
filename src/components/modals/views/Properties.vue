        ...
        <div class="modal-body">
            ...
            <template v-if="selectedItem.type === 'file'">
                ...
                <div class="row">
                    ...
                    <div class="col-9">
                        <span v-if="url || protected_url">{{ url || protected_url}}</span>
                        <span v-else>
                            <button v-on:click="getUrl" type="button"
                                    class="btn btn-sm btn-light">
                                <i class="fas fa-sm fa-link"></i> Get URL
                            </button>
                            <!-- <button v-on:click="getProtectedUrl" type="button"
                                    class="btn btn-sm btn-light">
                                <i class="fas fa-sm fa-link"></i> Protected URL
                            </button> -->
                        </span>
                    </div>
                    <div v-if="url || protected_url" class="col-1 text-right">
                        <i v-on:click="copyToClipboard(url || protected_url)"
                           v-bind:title="lang.clipboard.copy"
                           class="far fa-copy"></i>
                    </div>
                </div>
